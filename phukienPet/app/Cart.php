...
class Cart{
    ...
    public function add($item, $product_id) {//item laf product, product_id la id
        $storedItem = ['qty'=>0,'product_id' => 0,'product_name' => $item->product_name,
            'product_price'=>$item->product_price,'product_image' => $item->product_image,'product_description' => $item->product_description,
            'item' => $item];
